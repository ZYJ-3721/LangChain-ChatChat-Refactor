...
from chatchat.settings.settings_utils import *
from typing import Dict, Tuple, Union, List, Literal, Optional, Any


# chatchat 数据目录，必须通过环境变量设置。如未设置则自动在当前目录创建chatchat_data
CHATCHAT_ROOT = Path(os.environ.get("CHATCHAT_ROOT", "chatchat_data")).resolve()
if not CHATCHAT_ROOT.exists(): # 如果不存在则自动创建
    CHATCHAT_ROOT.mkdir(parents=True, exist_ok=True)
...
class BasicSettings(BaseFileSettings):
    ...
    def make_dirs(self):
        '''创建所有数据目录'''
        for path in [self.DATA_PATH, self.LOG_PATH, self.MEDIA_PATH, self.BASE_TEMP_DIR, self.KB_ROOT_PATH]:
            path.mkdir(parents=True, exist_ok=True)
        for name in ["image", "audio", "video"]:
            (self.MEDIA_PATH / name).mkdir(parents=True, exist_ok=True)
        (self.BASE_TEMP_DIR / "openai_files").mkdir(parents=True, exist_ok=True)
...
class KBSettings(BaseFileSettings):
    ...
    DEFAULT_VS_TYPE: Literal["faiss", "milvus", "zilliz", "es", "pg", "relyt", "chromadb"] = "faiss"
    """默认向量库/全文检索引擎类型"""
    ...
    PDF_OCR_THRESHOLD: Tuple[float, float] = (0.7, 0.7)
    """
    PDF OCR 控制：只对宽高超过页面一定比例（图片宽/页面宽，图片高/页面高）的图片进行 OCR。
    这样可以避免 PDF 中一些小图片的干扰，提高非扫描版 PDF 处理速度
    """

    KB_INFO: Dict[str, str] = {"samples": "关于本项目issue的解答"} # TODO: 都存在数据库了，这个配置项还有必要吗？
    """每个知识库的初始化介绍，用于在初始化知识库时显示和Agent调用，没写则没有介绍，不会被Agent调用。"""

    kbs_config: Dict[str, Dict] = {
            "faiss": {},
            "milvus": {
                "host": "127.0.0.1",
                "port": "19530",
...
class PlatformConfig(MyBaseModel):
    ...
    platform_type: Literal["xinference", "ollama", "openai", "oneapi", "fastchat"] = "xinference"
    """平台类型"""
...
class ModelSettings(BaseFileSettings):
    ...
    MAX_TOKENS: Optional[int] = None # TODO: 似乎与 LLM_MODEL_CONFIG 重复了
    ...
    MODEL_PLATFORMS: List[PlatformConfig] = [
        PlatformConfig(**{
            ...
            "llm_models": ["qwen2-instruct", "qwen2-vl-instruct"],
            "embed_models": ["bge-large-zh-v1.5"],
            "rerank_models": ["bge-reranker-large"],
            "image2text_models": ["qwen2-vl-instruct"],
            "text2image_models": ["stable-diffusion-xl-base-1.0"],
            "speech2text_models": [],
            "text2speech_models": [],
        }),
        PlatformConfig(**{
...
class PromptSettings(BaseFileSettings):
    ...
    model_config = SettingsConfigDict(
        yaml_file=CHATCHAT_ROOT / "prompt_settings.yaml",
...
class ToolSettings(BaseFileSettings):
    """Agent 工具配置"""
    model_config = SettingsConfigDict(
        yaml_file=CHATCHAT_ROOT / "tool_settings.yaml",
...
class SettingsContainer:
    ...
    def createl_all_templates(self):
        ...
        self.tool_settings.create_template_file(write_file=True)
...
if __name__ == "__main__":
    ...
    """run in terminal:
    cd LangChain-ChatChat-Refactor
    python -m chatchat.settings.settings
    """
